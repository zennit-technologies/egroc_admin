...
if (isset($_POST['delete_other_images']) && $_POST['delete_other_images'] == 1) {
    ...
    */
    ...
    if (empty($_POST['seller_id']) || empty($_POST['product_id']) || $_POST['image'] == '') {
        $response['error'] = true;
...
if (isset($_POST['get_delivery_boys']) && !empty($_POST['get_delivery_boys'] == 1)) {
    ...
    $sql = "SELECT count(id) as total FROM `delivery_boys` WHERE status=1" . $where;
    $db->sql($sql);
    $total = $db->getResult();

    $sql = "SELECT * FROM delivery_boys WHERE status=1 " . $where . " ORDER BY id DESC";
    $db->sql($sql);
    $res1 = $db->getResult();
    if (!empty($res1)) {
        for ($i = 0; $i < count($res1); $i++) {
