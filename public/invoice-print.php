...
// echo $sql_outer = "SELECT oi.price as order_item_price,oi.id as order_item_id,oi.*,u.*,p.*,v.*,o.*,u.name as uname,oi.discounted_price as dis_price,d.name as delivery_boy,o.status as order_status,oi.active_status as order_item_status,p.name as pname,(SELECT short_code FROM unit un where un.id=v.measurement_unit_id)as mesurement_unit_name FROM `order_items` oi JOIN users u ON u.id=oi.user_id JOIN product_variant v ON oi.product_variant_id=v.id JOIN products p ON p.id=v.product_id JOIN orders o ON o.id=oi.order_id LEFT JOIN delivery_boys d ON oi.delivery_boy_id=d.id WHERE o.id=" . $ID;
...
<section class="content">
    ...
    <section class="invoice">
        ...
        <?php if ($res_outer[0]['discount'] > 0) {
            ...
            // $discounted_amount = $res_total[0]['total'] *  / 100; /*  */
