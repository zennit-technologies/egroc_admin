...
<div class="content-wrapper">
    ...
    $sql = "SELECT value FROM settings where variable='seller_privacy_policy'";
    $db->sql($sql);
        ...
        if ($permissions['settings']['update'] == 1) {
            if (!empty($_POST['privacy_policy']) || !empty($_POST['terms_conditions'])) {
                ...
                if (empty($res_privacy)) {
                    $sql = "INSERT INTO settings (variable,value) VALUES ('seller_privacy_policy','" . $privacy_policy . "')";
                } else {
                    $sql = "UPDATE `settings` SET `value`='" . $privacy_policy . "' WHERE `variable` = 'seller_privacy_policy'";
                }
                $db->sql($sql);
                $sql = "SELECT value FROM settings where variable='seller_terms_conditions'";
                $db->sql($sql);
                $res_terms = $db->getResult();

                if (empty($res_terms)) {
                    $sql = "INSERT INTO settings (variable,value) VALUES ('seller_terms_conditions','" . $terms_conditions . "')";
                } else {
                    $sql = "UPDATE `settings` SET `value`='" . $terms_conditions . "' WHERE `variable` = 'seller_terms_conditions'";
                }
                $db->sql($sql);
    ...
    <section class="content-header">
        <h1>Seller Privacy Policy And Terms & Conditions</h1>
        <h4><?= $message ?></h4>
        <ol class="breadcrumb">
        ...
        <div class="row">
            ...
            <div class="col-md-4" style="margin-bottom:10px;">
                <?php if ($permissions['settings']['read'] == 1) { ?>
                    <a href='seller-play-store-privacy-policy.php' target='_blank' class='btn btn-primary btn-sm'>Seller Privacy Policy Page for Play Store</a>
                <?php } ?>
            </div>
            <div class="col-md-4" style="margin-bottom:10px;">
                <?php if ($permissions['settings']['read'] == 1) { ?>
                    <a href='seller-play-store-terms-conditions.php' target='_blank' class='btn btn-primary btn-sm'>Seller Terms Conditions Page for Play Store</a>
                <?php } ?>
            </div>
        </div>
