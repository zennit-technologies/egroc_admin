...
if (isset($_POST['get_orders']) && !empty($_POST['get_orders'])) {
    ...
    $sql = "select count(DISTINCT(order_id)) as total from order_items oi where oi.delivery_boy_id=" . $delivery_boy_id . $where;
    // echo "test";
    // echo $sql;
    $db->sql($sql);
    $res = $db->getResult();
    ...
    if (!empty($item_ids) && !empty($postStatus)) {
        ...
        for ($i = 0; $i < count($item_ids); $i++) {
            ...
            if ($res_order_item[0]['active_status'] == 'awaiting_payment' && ($postStatus == 'returned' || $postStatus == 'delivered' || $postStatus == 'shipped' || $postStatus == 'processed')) {
                $response['error'] = true;
                $response['message'] = "Order item can not be $postStatus. Because it is on awaiting status. Order item ID " . $item_ids[$i];
                print_r(json_encode($response));
                return false;
            }
            ...
            if ($fn->is_order_item_cancelled($item_ids[$i])) {
                $response['error'] = true;
                $response['message'] = '<p class="alert alert-danger">Could not update order status  cancelled or returned! please check order item ID ' . $item_ids[$i] . '</p>';
                print_r(json_encode($response));
                return false;
            }
            ...
            if ($postStatus == 'cancelled') {
                $cancelation_error = 0;
                $resp = $fn->is_product_cancellable($item_ids[$i]);
                if ($resp['till_status_error'] == 1 || $resp['cancellable_status_error'] == 1) {
                    $cancelation_error = 1;
                }
                if ($cancelation_error == 1) {
                    $resp['error'] = true;
                    $resp['message'] = "Order item ID " . $item_ids[$i] . " is not cancelable or not matching cancelation criteria!</p>";
            ...
            $db->update('order_items', $final_status, 'id=' . $item_ids[$i]);
        }
        $response['error'] = false;
